...
namespace JojaExpress
{
    public class ItemDeliver
    {
        ...
        public ItemDeliver(IDictionary<ISalable, ItemStockInformation> toBeDelivered, string packageId)
        {
            ...
            current = new Vector2(target.X + Game1.viewport.Width, target.Y);
        ...
        public bool draw(SpriteBatch b)
        {
            ...
            if (current.X - Game1.viewport.X < -24)
            {
                ...
                targetLocation.debris.Add(Game1.createItemDebris(new PackedItem(packageId, toBeDelivered), new Vector2(target.X, target.Y), 0));
                droped = true;
            }
            return false;
        }
